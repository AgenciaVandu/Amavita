    ...
    <div class="px-6 mt-6" x-data="{open:true}">
        <div class="bg-gray-50 shadow-lg mb-4">
            <div class="py-1 px-2 bg-gray-200 rounded rounded-t-lg mb-2 cursor-pointer hover:bg-gray-100"
                ...
                <h2 class="text-xl text-gray-800 font-bold">Seccion Master</h2>
            </div>
            <div x-show="open">
                    ...
                    <div class="flex flex-wrap p-4">
                        @if ($master->resources->first())
                            <div class="shadow-md bg-white rounded-lg mx-2 mt-4">
                                <img src="{{ Storage::url($master->resources->first()->url) }}" loop muted preload
                                    autoplay class="h-40 w-60"></img>
                                <div class="flex justify-between items-center py-2 mx-2">
                                    <i class="far fa-image ml-2"></i>
                                    <i wire:click="delete({{ $master->resources->first()->id }})"
                        ...
                        @if ($file)
                            <div class="shadow-md bg-white rounded-lg mx-2 mt-4">
                                <img src="{{ $file->temporaryUrl() }}" loop muted preload autoplay
                                    class="h-40 w-60 opacity-30"></img>
                            </div>
                        @endif
